...
- **Сканирование сети**: Обнаружение активных хостов и открытых портов; с v1.4.0 — сканирование **нескольких подсетей** (`custom_subnets`).
...
- **Пользовательский интерфейс**: отображение на e-Paper HAT **и/или веб-интерфейс** — с v1.4.0 **headless-режим** (`epd_type:"none"`, без HAT).
- **Веб-интерфейс**: страница логина + сессионные cookie (v1.4.0), мониторинг, управление конфигурацией и системой.
- **Стабильность** (v1.4.0): singleton EPDManager с защитой SPI и авто-recovery, мониторинг здоровья (потоки/RSS/FD), блокировка запуска двух экземпляров.
...
- Имя пользователя и hostname установлены в `bjorn`.
- e-Paper HAT 2.13 дюйма подключён к GPIO-пинам.
- **Минимальная версия Bjorn**: v1.4.0+ (мульти-источник platform detection для BCM2712, headless-режим, login-аутентификация).
...
| RPi 5 (BCM2712) | ✅ Поддержка (протестировано в форке) | **v1.4.0+** |
...
до v1.3.1 падают на определении платы. Используй v1.4.0+ для
