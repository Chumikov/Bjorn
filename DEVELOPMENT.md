...
- **Текущая версия**: v1.3.4 (см. `version.txt`).
...
Текущее состояние (v1.3.4): **280 тестов**, покрытие ~27.8% (gate
`--cov-fail-under=25` в `pytest.ini`). Целевой ориентир — 50% (запланирован
на v1.4.0 через backfill тестов для `Bjorn.py`, `orchestrator.py`,
`display.py`, `comment.py`).

Важно различать两类 тестов:
...
- **Аутентификация**: HTTP Basic Auth (WEB-8). Логин/пароль по умолчанию
  `admin` / `bjorn` — хранятся в `config/shared_config.json` (ключи
  `web_username` / `web_password` / `web_auth_enabled` / `web_bind_address`).
  Смените пароль перед deploy в сеть. Подробности — [SECURITY.md](SECURITY.md).
- **Возможности**:
